...
Cada fila numerada (**#1**, **#2**, …) en [`changes/migracion-react-a-angular/migration-catalog.md`](./changes/migracion-react-a-angular/migration-catalog.md) es una unidad de trazabilidad para **Foreach**: un agente o iteración puede tomar **un #** a la vez (todo el lote repartiendo ítems). La columna **Tipo** marca **#1** como único `shell` (marco global SPA); el resto son `feature`. La **lista plana** al final del catálogo ordena paths del legado **shell primero**, luego features por dependencia (**#4** → **#3** → **#2** → **#6** → **#5**). [`tasks.md`](./changes/migracion-react-a-angular/tasks.md) alinea olas a esas filas. Los requisitos de paridad frente al legado están en `proposal.md`, `design.md` y deltas bajo `changes/…/specs/`.
