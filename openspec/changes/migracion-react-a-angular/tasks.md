...
- [x] 0.8 Catálogo: columna **Tipo** (`shell` en **#1** únicamente; `feature` en **#2–#6**); sección **Lista plana para Foreach** (orden shell → **#4** → **#3** → **#2** → **#6** → **#5**) en [migration-catalog.md](./migration-catalog.md).
...
- [ ] 1.2 Completar en [migration-catalog.md](./migration-catalog.md) columnas **Ruta(s) legado**, **Paths componentes legado**, **APIs** para **#1–6**; rellenar la **Lista plana para Foreach** con paths reales del legado (una línea por path bajo cada **#**); añadir filas si el SPA legado tiene más pantallas que una sola lista.
