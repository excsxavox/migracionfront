...
6. `LEGACY_PATH_PENDING` — feature — destino previsto **#5** *por inventario* bajo `src/app/features/…` (una línea Foreach por pantalla/módulo adicional tras **Ola 1**; repetir este ítem de la cola global tantas veces como filas **#5** o subfilas del catálogo). **Hecho en destino (2026-04-23), primer sub-ítem sin baseline legado:** `src/app/features/cotizaciones/cotizaciones.routes.ts` (ruta `acerca`); `src/app/features/cotizaciones/pages/cotizaciones-readme/`; enlace «Acerca» en `src/app/shell/layout/main-layout.component.html`. Hasta inventario: placeholder; tras inventario: p. ej. `src/app/features/<dominio>/<dominio>.routes.ts`; `src/app/features/<dominio>/pages/…`; cambios en `src/app/app.routes.ts` o shell (**#1**) si la nueva ruta enlaza desde nav global.
